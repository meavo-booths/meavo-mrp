...
| First-time user | [`docs/oauth-setup.md`](./docs/oauth-setup.md) | Google Cloud OAuth client, paste into Supabase. |
| First-time user | [`docs/domain-setup.md`](./docs/domain-setup.md) | One CNAME record for `stock.yourcompany.bg`. |
| Devops | [`docs/supabase-setup.md`](./docs/supabase-setup.md) | SQL: storage buckets, RLS, auth.users trigger. |
...
# fill in Supabase + Gemini keys (see docs/accounts-checklist.md)
...
- **Supabase** — Postgres, file storage, auth (Google SSO via OAuth)
...
src/
  app/
    ...
    auth/callback/            # Supabase OAuth callback (locale-agnostic)
  ...
  lib/
    auth/                     # Supabase server/browser clients + session helper
...
docs/                         # accounts, oauth, domain, supabase, zeron, deploy, operator
...
| `pnpm db:push` | Apply current schema directly (dev) |
